...
$files = $db->pselect("SELECT c.CandID, c.PSCID, c.IBISID, c.CandidateGUID,
    ...
    AND f.File like \"%bold%\"
    AND f.File like \"%native%\"
    ORDER BY c.PSCID", array());

$target_dir = "bold";  // WHERE f.File not like \"%DTI65%\"
...
foreach($files as $row) {
    ...
    createPath("/home/gluneau/pinch/swarfield/". $target_dir ."/" . substr($file, 0, strrpos($file, '/', -2) + 1 ));

    if (file_exists("/home/gluneau/pinch/swarfield/". $target_dir ."/" .  $file)) {
        print "s:";
    } else {
        copy("/data/ibis/data/" . $row["File"], "/home/gluneau/pinch/swarfield/". $target_dir ."/" .  $file);
        print "n:";
    }
