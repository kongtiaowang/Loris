...
pcc.Value as subjectkey, CASE c.Sex WHEN 'Male' THEN 'M' WHEN 'Female' THEN 'F' END AS Gender, 
...
flag f on f.SessionID = s.ID join tsi i on i.CommentID=f.CommentID 
join participant_status ps on ps.CandID = c.CandID JOIN participant_status_options po on po.ID=ps.participant_status
...
AND COALESCE(s.Screening, '') 
NOT IN ('Failure', 'Withdrawal') 
AND f.Administration='All' 
AND f.Data_entry='Complete' 
AND EXISTS (SELECT 'x' FROM flag df WHERE df.CommentID=CONCAT('DDE_', i.CommentID) AND df.Data_entry='Complete') AND NOT EXISTS (SELECT 'x' FROM conflicts_unresolved cu WHERE cu.CommentId1=i.CommentID OR cu.CommentId2=i.CommentID) AND s.CenterID IN (2, 3, 4, 5) AND ( ps.study_consent = 'yes' AND ps.study_consent_withdrawal IS NULL) 
AND (ps.ndar_consent = 'yes' AND ps.ndar_consent_withdrawal IS NULL) 
and c.ProjectID = $id AND COALESCE(pcc.Value, '') <> '' 
AND po.Description NOT IN ('Excluded', 'Ineligible','Refused/Not Enrolled')";
