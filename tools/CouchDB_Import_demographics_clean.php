                                 ...
                                 LEFT JOIN DSMIV_checklist dsm
                                       ON ( dsm.CommentID = f.CommentID )
                                           AND EXISTS (
                                               SELECT 'x'
                                               FROM flag
                                               WHERE flag.CommentID = CONCAT('DDE_', dsm.CommentID)
                                                   AND flag.Data_entry = 'Complete'
                                           ) AND NOT EXISTS (
                                               SELECT 'x'
                                               FROM conflicts_unresolved cu
                                               WHERE dsm.CommentID=cu.CommentId1 OR dsm.CommentID=cu.CommentId2
                                           )
                                 LEFT JOIN (
                                       ...
                                       AND EXISTS (
                                            SELECT 'x'
                                            FROM flag tf
                                            WHERE tf.CommentID = CONCAT('DDE_', flag.CommentID)
                                                AND tf.Data_entry = 'Complete'
                                       ) AND NOT EXISTS (
                                            SELECT 'x'
                                               FROM conflicts_unresolved cu
                                               WHERE flag.CommentID=cu.CommentId1 OR flag.CommentID=cu.CommentId2
                                       )
    ...
    function getADOSModule($SessionID)
    {
        $rows = $this->SQLDB->pselect(
            ...
            " AND EXISTS (
                 SELECT 'x'
                 FROM flag f
                 WHERE f.CommentID = CONCAT('DDE_', flag.CommentID)
