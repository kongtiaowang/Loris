...
class CouchDBDemographicsImporter {
    ...
    function _generateQuery() {
        $config = NDB_Config::singleton();
        $fieldsInQuery = "SELECT c.DoB, c.CandID, c.PSCID, s.Visit_label, s.SubprojectID, p.Alias as Site, c.Gender, s.Current_stage, CASE WHEN s.Visit='Failure' THEN 'Failure' WHEN s.Screening='Failure' THEN 'Failure' WHEN s.Visit='Withdrawal' THEN 'Withdrawal' WHEN s.Screening='Withdrawal' THEN 'Withdrawal' ELSE 'Neither' END as Failure, c.ProjectID, c.flagged_caveatemptor as CEF, c.flagged_caveatemptor as CEF, c_o.Description as CEF_reason, c.flagged_other as CEF_comment, c.flagged_date as CEF_date, pc_comment.Value as Comment, COALESCE(pso.Description,'Active') as Status, ps.participant_suboptions as Status_reason, ps.reason_specify as Status_comments, ps.ndar_consent as NDAR_consent, COALESCE(ps.ndar_consent_withdrawal,'0000-00-00') AS NDAR_consent_withdrawal, ps.study_consent as Study_consent, COALESCE(ps.study_consent_withdrawal,'0000-00-00') AS Study_consent_withdrawal,ps.air_consent as Air_pollution_consent, COALESCE(ps.air_consent_withdrawal,'0000-00-00') AS Air_pollution_consent_withdrawal,ps.mail_toothkit_consent as Mail_tooth_kit_consent, COALESCE(ps.mail_toothkit_consent_withdrawal,'0000-00-00') AS Mail_tooth_kit_consent_withdrawal";
