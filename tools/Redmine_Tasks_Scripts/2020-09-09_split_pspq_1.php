 ...
 * Splits and migrates pspq_1 data into pspq_SelfReport and pspq_InformantReport
 *
 * PHP Version 7
 *
 * @category Main
...
if ($confirm === false && $revert === false) {
    ...
    /*
     * PSPQ_1 DATA WHERE RESPONDENT IS FEMALE AND IS THE MOTHER/STEPMOTHER/OTHER
     */

    // Multidimensional array where
    // $female_comment_ids[$i][0] = CommentID from pspq_1
    ...
    $pspq_1_female_comment_ids_array = $db->pselect(
        "SELECT
            CommentID
        FROM
            pspq_1
    ...
    $pspq_1_female_count = count($pspq_1_female_comment_ids_array);

    $i = 0;

    foreach ($pspq_1_female_comment_ids_array as $cid) {
        $is_dde = false;
    ...
    /*
     * PSPQ_1 DATA WHERE RESPONDENT IS MALE AND IS THE FATHER/STEPFATHER/OTHER
     */

    // Multidimensional array where
    // $male_comment_ids[$i][0] = CommentID from pspq_1
    // $male_comment_ids[$i][1] = CommentID for pspq_SelfReport_dad
    // $male_comment_ids[$i][2] = CommentID for pspq_InformantReport_dad_on_mom
    $male_comment_ids;

    // All pspq_1 comment ids where respondent is male
    $pspq_1_male_comment_ids_array = $db->pselect(
        "SELECT
            CommentID
        FROM
            pspq_1
    ...
    $pspq_1_male_count = count($pspq_1_male_comment_ids_array);

    $i = 0;

    foreach ($pspq_1_male_comment_ids_array as $cid) {
        $is_dde = false;
        ...
        $new_cid_self_report      = replaceTestID($old_cid, $test_id_pspq_1, $test_id_self_dad);
        $new_cid_informant_report = replaceTestID($old_cid, $test_id_pspq_1, $test_id_informant_dad_on_mom);
    ...
    echo "\n# of rows to be transferred to PSPQ MOM tables: " . $pspq_1_female_count .
         "\n# of rows to be transferred to PSPQ DAD tables: " . $pspq_1_male_count . "\n";
...
if ($confirm) {
    ...
    echo "\n* # OF ROWS TRANSFERRED FROM PSPQ_1 TO: *" .
