...
// Multidimensional array containing the CommentIDs of pspq1, and it's converted CommentIDs for pspq_SelfReport_mom and pspq_InformantReport_mom_on_dad
...
if ($confirm === false) {
    ...
    // prints old CommentID and it's corresponding new CommentID
    pspqMom_part1(true, $db);
...
if ($confirm) {
    ...
    $all_comment_ids = pspqMom_part1(false, $db);
    pspqMom_part2($all_comment_ids, $num_rows_pspq_1 , $db);
...
/**
 * Converts old CommentID to new CommentID, stores it in $all_comment_ids, and prints it
 * Called when script without confirm is run
 *
 * @param Array $pspq_1_comment_ids_array Array of all the CommentIDs in pspq_1 table
 */
function pspqMom_part1 ($echo, $db) {

    global $test_id_pspq_1, $test_id_self_mom, $test_id_informant_mom_on_dad;
    $all_comment_ids;
    ...
    foreach ($pspq_1_comment_ids_array as $cid) {
        ...
        if ($echo) {
            echo "\nData in pspq_1 (CommentID " . $old_cid . ") will be inserted into\n" .
...
/**
 * Converts old CommentID to new CommentID, stores it in $all_comment_ids, and prints it
 * Called when script with confirm is run
 *
 * @param Database $db Database Singleton
 */
function pspqMom_part2 ($all_comment_ids, $count, $db) {

    global $num_rows_pspq_1;
    global $num_rows_pspq_self_mom, $num_rows_pspq_informant_mom;
    global $flag_rows_pspq_1, $flag_rows_pspq_self_mom, $flag_rows_pspq_informant_mom;
    global $pspq_self_cols, $pspq_informant_cols;

    for ($i = 0; $i < $count; $i++) {
        echo $count;

        $old_cid = $all_comment_ids[$i][0];                  // CommentID in pspq_1
        $new_cid_self_report = $all_comment_ids[$i][1];      // CommentID for pspq_self
        $new_cid_informant_report = $all_comment_ids[$i][2]; // CommentID for pspq_informant

        echo "\n" . $old_cid . "\n" . $new_cid_self_report . "\n" . $new_cid_informant_report . "\n";
        ...
        echo $row["q5_information_than_socialize"] . "\n";
        ...
        echo "\nData in pspq_1 (CommentID " . $old_cid . ") was transferred to\n" .
        "-> pspq_SelfReport_mom and flag (CommentID " . $new_cid_self_report . ")\n" .
        "-> pspq_InformantReport_mom_on_dad and flag (CommentID " . $new_cid_informant_report . ")\n";
    }
