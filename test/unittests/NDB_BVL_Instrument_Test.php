...
class NDB_BVL_Instrument_Test extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->_module = $this->getMockBuilder('Module')->disableOriginalConstructor()->getMock();
        $this->_Instrument = new NDB_BVL_Instrument($this->_module,"t", "e", "s","t");
     ...
     */
    public function testDetermineDataEntryAllowedFalse()
    {
        $this->_dbMock->expects($this->once())
            ->method('pselectOne')
            ->willReturn('Complete');       
        $this->assertFalse($this->_Instrument->determineDataEntryAllowed($this->_dbMock));
    }
    /**
     * Test select() method retrieves all _candidate and related info
     */
    public function testDetermineDataEntryAllowedTrue()
    {
        $this->_dbMock->expects($this->once())
            ->method('pselectOne')
            ->willReturn('test');
        $this->assertTrue($this->_Instrument->determineDataEntryAllowed($this->_dbMock));
    }
