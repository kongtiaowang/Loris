...
if (isset($_REQUEST['sessionID'])) {
    ...
    if (in_array($project, $data_set_ibis_1_2)) {
        // Here we have double query, I think it is not important. The advantage is to simplify the modification.
        $query = "SELECT c.CandID, s.Visit_label FROM session AS s, candidate AS c
                WHERE s.ID=:SID AND s.CandID=c.CandID AND s.Active='Y'";

        $qparams = array('SID' => $sessionID);
    ...
    if (in_array($project, $data_set_ibis_ep_ds)) {
        $group = GROUP_IBIS_EP_DS;
    }
}

if ($group == GROUP_IBIS_EP_DS) {

    // instantiate feedback mri object
    $comments = new FeedbackMRI_IBIS($_REQUEST['fileID'], $_REQUEST['sessionID']);

    $tpl_data['saved'] = false;

    $errors = [];

    // get the currently saved comments
    $saved_comments = $comments->getComment();
    ...
    if (isset($_POST['fire_away']) && $_POST['fire_away'] && $user->hasPermission('imaging_browser_qc')) {
        ...
        $saved_comments = array_merge($saved_comments, $_POST);
    }
    ...
    $comment_types = $comments->getCommentItems();

    // loop through the comment types
    $i = 0;
    foreach ($comment_types AS $comment_type_label => $comment_array) {
        $CommentTpl =& $tpl_data['comment'][$i];

        $CommentTpl['label'] = $comment_type_label;

        // loop through the predefined comments
        $j = 0;
        foreach ($comment_array AS $id => $itm) {
            $tpl_item =& $CommentTpl['itms'][$j];
            $tpl_item['id'] = $id;
            $tpl_item['label'] = $itm['l'];
            $tpl_item['type'] = $itm['t'];
            if ($itm['t'] == 'select') {
                if (isset($errors[$id])) {
                    $tpl_item['error'] = $errors[$id];
                }
                $tpl_item['selected'] = $saved_comments[$id] ?? '';
                $tpl_item['options'] = $itm['o'];
            }
            elseif ($itm['t'] == 'text') {
                if (!is_null($saved_comments)) {
                    if (isset($saved_comments[$id])) {
                        $tpl_item['savedtext'] = $saved_comments[$id];
                    }
                }
                else {
                    $tpl_item['savedtext'] = '';
    ...
    $page_template = 'feedback_mri_popup_ibis.tpl';
}
else {// GROUP_LORIS_DEFAULT part
    ...
    $page_template = 'feedback_mri_popup.tpl';
}
