...
$(document).ready(function () {
  ...
  const getSurveys = (visit) => {
    $.get(loris.BaseURL + "/survey_accounts/ajax/GetAvailableSurveys.php", {
      ...
      function(result) {
          result = JSON.parse(result);
          if (result.option_string == null) {
            $('[name="Test_name[]"]').html(
              '<option>-- Enter valid CandID, PSCID and visit label values to see the list of available surveys --</option>'
            );
          } else {
  ...
  $("input[name=CandID]").on('change keyup paste',function() {
      getSurveys($("select[name=VL]").val());
  });
  // IBIS SPECIFIC OVERRIDE CODE ENDS HERE
});
