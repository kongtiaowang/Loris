...
window.addEventListener('load', () => {
    function loadCards(candidate, extraprops) {
        console.log('loadCards closure');
        return async (visits) => {
            console.log('in closure', visits);
            ...
            allprops = { ...baseprops, ...modprops, ...extraprops };
    ...
    ReactDOM.render(
        React.createElement(
            lorisjs.portal.index.IndexPage,
            {
                'BaseURL' : loris.BaseURL,
                'loadCardsClosure': loadCards,
            }
        ),
    ...
    lorisjs.portal.index.indexPage(loris.BaseURL, {$candidate->getCandID()}, loadCards);
    {*
    let candidate = null;
