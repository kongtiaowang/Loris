...
// IBIS CODE OVERRIDE STARTS HERE
// scqc_pa_su option will be kept when user choose ep-v36, otherwise keep scq_subject_survey

if ($_REQUEST['VL'] != 'EP-V36') {
        $subKey = 'Test_name';
        // remove scq current
    $valueToRemove = 'scqc_pa_su';

    foreach ($result as $key => $innerArray) {
        if (isset($innerArray[$subKey]) && $innerArray[$subKey] === $valueToRemove) {
             unset($result[$key]);
        }
    }
} else {
    // remove scq lifetime
    $subKey        = 'Test_name';
    $valueToRemove = 'scq_subject_survey';
