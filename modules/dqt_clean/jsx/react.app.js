...
class DataQueryApp extends Component {
      ...
      if (this.state.queryIDs.hasOwnProperty(key)) {
        for (let i = 0; i < this.state.queryIDs[key].length; i += 1) {
          ...
          curRequest = Promise.resolve(
            $.ajax(loris.BaseURL
              + '/AjaxHelper.php?Module=dqt_clean&script=GetDoc.php&DocID='
  ...
  async handleProgressBarSetup(callback) {
    const response = await fetch(
      `${loris.BaseURL}/dqt_clean/dqt_setup/?format=json`,
  ...
  async requestSessions(callback) {
    const response = await fetch(
      `${loris.BaseURL}/dqt_clean/sessions/?format=json`,
      {credentials: 'same-origin', method: 'GET'}
    );
    const reader = await response.body.getReader();
    let chunks = ''; // array of received binary chunks (comprises the body)
  ...
  saveCurrentQuery(name, shared, override) {
    ...
    $.post(loris.BaseURL
      + '/AjaxHelper.php?Module=dqt_clean&script=saveQuery.php', {
  ...
  loadFilterRule(rule) {
    ...
    $.ajax({
      url: loris.BaseURL
        + '/AjaxHelper.php?Module=dqt_clean&script=datadictionary.php',
      success: (data) => {
        rule.fields = data;
      },
      async: false,
    ...
    for (let i = 0; i < fields.length; i += 1) {
      ...
      if (DocTypes.indexOf(category) === -1) {
        ...
        $.ajax({
          type: 'POST',
          url: loris.BaseURL
            + '/AjaxHelper.php?Module=dqt_clean&script='
